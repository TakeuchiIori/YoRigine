...
// std::shared_ptr<Material> material) { 	auto it = nameToIndex_.find(name); 	if
//(it != nameToIndex_.end()) { 		return it->second; // 既にある
//	}
...
//	material->SetTextureFilePath(name + ".png"); // 仮：名前＋.png
//をテクスチャとして想定 	material->Initialize();
//
...
MaterialManager::GetAllMaterials() const {
  return materials_;
}
