...
public:
  ...
  // 音声を再生してハンドルを返す
  SoundHandle Play(const std::string &filepath, bool loop = false,
                   float volume = 1.0f,
                   SoundCategory category = SoundCategory::SE);
  SoundHandle Play(const std::wstring &filepath, bool loop = false,
                   float volume = 1.0f,
                   SoundCategory category = SoundCategory::SE);
  ...
  void PlayOneShot(const std::string &filepath, float volume = 1.0f,
                   SoundCategory category = SoundCategory::SE);
  void PlayOneShot(const std::wstring &filepath, float volume = 1.0f,
                   SoundCategory category = SoundCategory::SE);
...
public:
  ...
  struct SoundData {
    ...
    SoundData(SoundData &&other) noexcept
        : wfex(other.wfex), pBuffer(other.pBuffer),
...
private:
  ...
  // 再生処理の共通化
  IXAudio2SourceVoice *PlayInternal(const SoundData &soundData, bool loop,
