...
bool UIManager::RenameUI(const std::string &oldId, const std::string &newId) {
  ...
  // 選択ID更新
  if (selectedUIId_ == oldId)
    selectedUIId_ = finalId;
...
void UIManager::DrawAll() {
  ...
  for (const auto &id : drawOrder_) {
    ...
    if (it != uiElements_.end() && it->second) {
      // 所有者が自分で描くUIは、ここで描くと二重描画になるので除外する。
      if (it->second->IsSelfDrawn())
        continue;
...
void UIManager::SortByLayer() { RebuildDrawOrder(); }
...
bool UIManager::SaveScene(const std::string &sceneName) {
  ...
  try {
    ...
    for (const auto &id : drawOrder_) {
      auto it = uiElements_.find(id);
      if (it == uiElements_.end() || !it->second || it->second->IsTransient())
        continue;
...
bool UIManager::LoadScene(const std::string &sceneName) {
  ...
  if (sceneData.contains("groups")) {
    for (auto it = sceneData["groups"].begin(); it != sceneData["groups"].end();
         ++it) {
...
std::vector<std::string> UIManager::GetAvailableScenes() const {
  ...
  for (const auto &entry :
       std::filesystem::directory_iterator(SCENE_DIRECTORY)) {
...
void UIManager::AddToGroup(const std::string &groupName,
...
void UIManager::RemoveFromGroup(const std::string &groupName,
                                const std::string &uiId) {
...
void UIManager::RebuildDrawOrder() {
  ...
  for (const auto &[id, ui] : uiElements_) {
    if (std::find(drawOrder_.begin(), drawOrder_.end(), id) ==
        drawOrder_.end()) {
  ...
  drawOrder_.erase(std::remove_if(drawOrder_.begin(), drawOrder_.end(),
                                  [this](const std::string &id) {
                                    return uiElements_.find(id) ==
                                           uiElements_.end();
                                  }),
                   drawOrder_.end());

  // 3.
  // レイヤー順でstable_sort(同一レイヤー内の相対順序=drawOrder_内の順序を維持)
  std::stable_sort(drawOrder_.begin(), drawOrder_.end(),
                   [this](const std::string &a, const std::string &b) {
                     auto itA = uiElements_.find(a);
                     auto itB = uiElements_.find(b);
                     if (itA == uiElements_.end() || itB == uiElements_.end())
                       return false;
