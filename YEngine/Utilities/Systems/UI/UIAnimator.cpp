...
// 基本アニメーション : 位置
// ============================================================
void UIAnimator::PlayPositionAnimation(const Vector3 &from, const Vector3 &to,
                                       float duration, Easing::Function easing,
                                       bool loop) {
  UIAnimation anim;
...
// 基本アニメーション : 拡縮
// ============================================================
void UIAnimator::PlayScaleAnimation(const Vector2 &from, const Vector2 &to,
                                    float duration, Easing::Function easing,
                                    bool loop) {
  UIAnimation anim;
...
// 基本アニメーション : 回転
// ============================================================
void UIAnimator::PlayRotationAnimation(const Vector3 &from, const Vector3 &to,
                                       float duration, Easing::Function easing,
                                       bool loop) {
  UIAnimation anim;
...
// 基本アニメーション : 色
// ============================================================
void UIAnimator::PlayColorAnimation(const Vector4 &from, const Vector4 &to,
                                    float duration, Easing::Function easing,
                                    bool loop) {
  UIAnimation anim;
...
void UIAnimator::StopClip(const std::string &clipName) {
  activeClips_.erase(std::remove_if(activeClips_.begin(), activeClips_.end(),
                                    [&](const ActiveClip &ac) {
                                      return ac.clip.name == clipName;
                                    }),
                     activeClips_.end());
}
...
// duration 到達後はループなら elapsed
// をラップ、非ループなら完了コールバック後に削除。
void UIAnimator::UpdateClips(float deltaTime) {
  if (!target_ || activeClips_.empty())
    return;
...
void UIAnimator::ApplyTrackValue(UIAnimTrackType type, float value) {
  ...
  case UIAnimTrackType::RotationZ: {
    auto r = target_->GetRotation();
    r.z = value;
    target_->SetRotation(r);
    break;
  }
  case UIAnimTrackType::R: {
    auto c = target_->GetColor();
    c.x = value;
    target_->SetColor(c);
    break;
  }
...
// プリセット : スライドイン
// ============================================================
void UIAnimator::PlaySlideIn(SlideDirection dir, float distance,
                             float duration) {
...
// プリセット : スライドアウト
// ============================================================
void UIAnimator::PlaySlideOut(SlideDirection dir, float distance,
                              float duration) {
...
// 制御 : 一時停止 / 再開
// ============================================================
void UIAnimator::PauseAnimation() { isPaused_ = true; }

void UIAnimator::ResumeAnimation() { isPaused_ = false; }
...
void UIAnimator::Update(float deltaTime) {
  ...
  for (size_t i = 0; i < animations_.size(); ++i) {
    ...
    // 区間の進み具合。折り返しの判定は必ずこの生の値で行う。
    // 反転後の t は復路で 1→0 と進むため、それで完了判定すると
    // 復路の終わりを検出できず、1往復した時点で固まってしまう。
    const float rawT = std::clamp(anim.elapsed / duration, 0.0f, 1.0f);

    // アニメーション完了チェック
    bool isComplete = false;
    // 往復時は進行を反転
    float t = anim.isReversing ? (1.0f - rawT) : rawT;
  ...
  for (auto it = completedIndices.rbegin(); it != completedIndices.rend();
       ++it) {
...
void UIAnimator::UpdateBasicAnimation(UIAnimation &anim, float t) {
  ...
  case UIAnimationType::Flip: {
    Vector2 scale;
    scale.x =
        anim.startScale.x + (anim.endScale.x - anim.startScale.x) * easedT;
    scale.y =
        anim.startScale.y + (anim.endScale.y - anim.startScale.y) * easedT;
    target_->SetScale(scale);
    break;
  }
  case UIAnimationType::Rotation: {
    Vector3 rot;
    rot.x = anim.startRotation.x +
            (anim.endRotation.x - anim.startRotation.x) * easedT;
    rot.y = anim.startRotation.y +
            (anim.endRotation.y - anim.startRotation.y) * easedT;
    rot.z = anim.startRotation.z +
            (anim.endRotation.z - anim.startRotation.z) * easedT;
  ...
  case UIAnimationType::Color: {
    Vector4 color;
    color.x =
        anim.startColor.x + (anim.endColor.x - anim.startColor.x) * easedT;
    color.y =
        anim.startColor.y + (anim.endColor.y - anim.startColor.y) * easedT;
    color.z =
        anim.startColor.z + (anim.endColor.z - anim.startColor.z) * easedT;
    color.w =
        anim.startColor.w + (anim.endColor.w - anim.startColor.w) * easedT;
...
void UIAnimator::UpdateShakeAnimation(UIAnimation &anim, float t) {
  ...
  float shake =
      sin(t * anim.frequency * 3.14159f * 2.0f) * anim.intensity * damping;
...
void UIAnimator::UpdateWobbleAnimation(UIAnimation &anim, float t) {
  ...
  float wobbleX =
      sin(t * anim.frequency * 3.14159f * 2.0f) * anim.intensity * (1.0f - t);
  float wobbleY = sin(t * anim.frequency * 3.14159f * 4.0f) * anim.intensity *
                  0.3f * (1.0f - t);
