...
public:
  ...
  // 所有者が自分で Draw を呼ぶUI。UIManager::DrawAll の対象から外れる。
  // シーンごとに描画レイヤーの扱いが違っても、確実に自前の順序で描きたいものに使う。
  void SetSelfDrawn(bool selfDrawn) { selfDrawn_ = selfDrawn; }
  ...
  // シンプルなアニメーション
  void PlayPositionAnimation(const Vector3 &from, const Vector3 &to,
                             float duration,
                             Easing::Function easing = Easing::Function::Linear,
                             bool loop = false);
  void PlayScaleAnimation(const Vector2 &from, const Vector2 &to,
                          float duration,
                          Easing::Function easing = Easing::Function::Linear,
                          bool loop = false);
  void PlayRotationAnimation(const Vector3 &from, const Vector3 &to,
                             float duration,
                             Easing::Function easing = Easing::Function::Linear,
                             bool loop = false);
  void PlayAlphaAnimation(float from, float to, float duration,
                          Easing::Function easing = Easing::Function::Linear,
                          bool loop = false);
  void PlayColorAnimation(const Vector4 &from, const Vector4 &to,
                          float duration,
                          Easing::Function easing = Easing::Function::Linear,
                          bool loop = false);
  ...
  void PlayFadeIn(float duration = 1.0f,
                  Easing::Function easing = Easing::Function::EaseOutQuad);
  void PlayFadeOut(float duration = 1.0f,
                   Easing::Function easing = Easing::Function::EaseInQuad);
  ...
  // アニメーション制御
  void StopAnimation(
      UIAnimationType type = UIAnimationType::None); // None指定で全停止
...
protected:
  ...
  // 所有者が自分で Draw を呼ぶUI（UIManager::DrawAll では描かない）
  ...
  // 読み込み直後にここ→sprite_ へ同期する（SyncSpriteToData /
  // ApplyDataToSprite）。
  // ============================================================
