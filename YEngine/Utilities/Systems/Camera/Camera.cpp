...
void Camera::UpdateMatrix() {
  ...
  // ------------------------------------------------------------
  worldMatrix_ =
      MakeAffineMatrix(transform_.scale, transform_.rotate, finalPos);
...
void Camera::InitializeCameraResource() {
  cameraResource_ =
      YoRigine::DirectXCommon::GetInstance()->CreateBufferResource(
          sizeof(CameraForGPU));
...
void Camera::UpdateShake() {
  ...
  if (cameraShake_.shakeTimer_ >= cameraShake_.shakeDuration_) {
    ...
    shakeOffset_.x =
        cameraShake_.shakeMinRange_.x +
        rndX * (cameraShake_.shakeMaxRange_.x - cameraShake_.shakeMinRange_.x);
    shakeOffset_.y =
        cameraShake_.shakeMinRange_.y +
        rndY * (cameraShake_.shakeMaxRange_.y - cameraShake_.shakeMinRange_.y);
  }
}
