...
public:
  ...
  /// <param
  /// name="targetPivot">プレイヤーの頭など、レイの始点となる注視点</param>
  ...
  const std::vector<FadeHit> &GetFadeHits() const { return fadeHits_; }
...
private:
  ...
  float cameraRadius_ =
      0.5f;                 // カメラの球体判定の半径（壁からどれくらい離すか）
  float avoidSpeed_ = 0.3f; // 壁が迫った時に手前に寄る素早さ (旧 60fps Lerp
                            // 係数。内部で /sec に換算)
  float returnSpeed_ = 0.05f; // 壁がなくなった時に元の距離に戻る素早さ (同上)
  float minDistanceRatio_ =
      0.15f; // カメラ最小距離率。これ以下にはピボットに寄せない
             // (ターゲットが画面いっぱいになるのを防ぐ)
  ...
  // --- フェードアルファ計算パラメータ ---
  // 遮蔽開始時の半透明度 (カメラが壁から離れている通常遮蔽)
  float occludeTargetAlpha_ = 0.25f;
  // カメラがこの深さだけ壁に侵入したら alpha=0 (完全透明) になる
  float fullyTransparentDepth_ = 0.5f;

  // このフレームにカメラレイが通過した全フェードコライダー。
  std::vector<FadeHit> fadeHits_;
};
