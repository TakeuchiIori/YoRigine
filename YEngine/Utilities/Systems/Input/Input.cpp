...
void Input::Initialize(WinApp *winApp) {
  ...
  HRESULT result;
  result =
      DirectInput8Create(winApp_->GethInstance(), DIRECTINPUT_HEADER_VERSION,
  ...
  assert(SUCCEEDED(result));
  // 排他制御レベルのセット
  result = keyboard->SetCooperativeLevel(winApp_->GetHwnd(),
...
/// </summary>
bool Input::BufferedKeyPress(BYTE keyNumber) { return TriggerKey(keyNumber); }
...
/// </summary>
/// <param
/// name="buttonNumber">マウスボタン番号(0:左,1:右,2:中,3~7:拡張マウスボタン)</param>
...
bool Input::IsTriggerMouse(int32_t buttonNumber) const {
  ...
  return ((mouse_.rgbButtons[buttonNumber] & 0x80) != 0) &&
         ((mousePre_.rgbButtons[buttonNumber] & 0x80) == 0);
}
...
/// マウスカーソルの表示・非表示を設定する
/// </summary>
void Input::SetMouseCursorVisibility(bool isVisible) { ShowCursor(isVisible); }
...
/// </summary>
void Input::SetJoystickDeadZone(int32_t stickNo, int32_t deadZoneL,
                                int32_t deadZoneR) {
  if (stickNo < 0 || stickNo >= devJoysticks_.size())
    return;
...
/// ジョイスティックの振動を設定する
/// </summary>
void Input::SetJoystickVibration(int32_t stickNo, uint16_t leftMotorSpeed,
...
/// </summary>
void Input::StartVibration(int32_t stickNo, float duration,
                           uint16_t leftMotorSpeed, uint16_t rightMotorSpeed) {
  if (stickNo < 0 || stickNo >= devJoysticks_.size())
    return;
...
float Input::GetJoystickAngle(int32_t stickNo) {
  ...
  float angle =
      atan2f(static_cast<float>(state.lY), static_cast<float>(state.lX)) *
      (180.0f / 3.14159265f);
  if (angle < 0) {
    angle += 360.0f;
  }
...
/// </summary>
size_t Input::GetNumberOfJoysticks() { return size_t(); }
...
bool Input::IsPadPressed(int32_t playerIndex, GamePadButton button) const {
  if (playerIndex < 0 || playerIndex >= devJoysticks_.size())
    return false;
  ...
  return (joystick.state_.xInput_.Gamepad.wButtons &
          static_cast<WORD>(button)) != 0;
}
...
bool Input::IsPadTriggered(int32_t playerIndex, GamePadButton button) const {
  ...
  return ((joystick.state_.xInput_.Gamepad.wButtons &
           static_cast<WORD>(button)) != 0) &&
         ((joystick.statePre_.xInput_.Gamepad.wButtons &
           static_cast<WORD>(button)) == 0);
}
...
bool Input::IsLeftStickMoving() {
  ...
  return (std::fabs(leftStickX) > deadZoneThreshold ||
          std::fabs(leftStickY) > deadZoneThreshold);
}
...
bool Input::IsRightStickMoving() {
  ...
  // 入力が閾値を超えているか判定
  const float deadZoneThreshold = 0.1f; // スティックの動きを無視する閾値
  return (std::fabs(rightStickX) > deadZoneThreshold ||
          std::fabs(rightStickY) > deadZoneThreshold);
}
