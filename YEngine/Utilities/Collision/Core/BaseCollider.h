...
// SphereCollider / AABBCollider / OBBCollider / CapsuleCollider
// の基盤となるクラス
class BaseCollider {
protected:
  ///************************* 基本処理 *************************///
...
public:
  ...
  using CollisionCallback =
      std::function<void(BaseCollider *self, BaseCollider *other)>;
  using DirectionalCollisionCallback = std::function<void(
      BaseCollider *self, BaseCollider *other, HitDirection dir)>;
...
public:
  ...
  void SetOnDirectionCollision(DirectionalCollisionCallback cb) {
    directionCallback_ = cb;
  }

  void SetOnEnterDirectionCollision(DirectionalCollisionCallback cb) {
    enterDirectionCallback_ = cb;
  }

  // 衝突開始時のコールバック呼び出し
  void CallOnEnterCollision(BaseCollider *other) {
    if (enterCallback_)
      enterCallback_(this, other);
  }

  // 衝突中のコールバック呼び出し
  void CallOnCollision(BaseCollider *other) {
    if (collisionCallback_)
      collisionCallback_(this, other);
  }

  // 衝突終了時のコールバック呼び出し
  void CallOnExitCollision(BaseCollider *other) {
    if (exitCallback_)
      exitCallback_(this, other);
  }

  // 衝突方向付きのコールバック呼び出し
  void CallOnDirectionCollision(BaseCollider *other, HitDirection dir) {
    if (directionCallback_)
      directionCallback_(this, other, dir);
  }

  // 衝突開始時の方向付きコールバック呼び出し
  void CallOnEnterDirectionCollision(BaseCollider *other, HitDirection dir) {
    if (enterDirectionCallback_)
      enterDirectionCallback_(this, other, dir);
  }
...
public:
  ...
  // BattleEnemy / MagicAttackInstance などへ戻す。BaseObject 前提にしないため
  // raw に留める。
  void SetOwnerRaw(void *owner) { ownerRaw_ = owner; }
  template <typename T> T *GetOwnerAs() const {
    return static_cast<T *>(ownerRaw_);
  }
  ...
  // カメラ外チェックフラグ設定
  // true  : 視錐台外に出たら CollisionManager の BroadPhase 登録をスキップ
  // (既定) false : 視錐台外でも常に当たり判定を回す (Player / ボス /
  // 落下死トリガーなど)
  ...
  // ─── レイヤーマスク (Unity 風) ──────────────────────────────
  // layerBits_     : このコライダー自身がどの層に属するか (通常は1ビット,
  // 複数ビット可) collisionMask_ : どの層と当たるか (ビット ON で反応)
  ...
  void SetPreviousCenter(const Vector3 &p) {
    previousCenter_ = p;
    hasPreviousCenter_ = true;
  }
  ...
  // カメラ遮蔽時のフェードモード。
  // true: このコライダーがカメラと player
  // の間に入った時、カメラをずらすのではなく
  //       オブジェクトを半透明にする (CameraCollisionResolver が
  //       フェードヒット一覧に格納)。
  // false(既定): 従来通りカメラをずらす。
  void SetCameraFadeMode(bool fade) { isCameraFadeMode_ = fade; }
  bool IsCameraFadeMode() const { return isCameraFadeMode_; }
...
private:
  ...
  // カメラ遮蔽フェードモード (SetCameraFadeMode で設定)
  bool isCameraFadeMode_ = false;
