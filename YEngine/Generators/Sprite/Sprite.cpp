...
void Sprite::Update() {
  ...
  Matrix4x4 uvTranslateMatrix =
      MakeTranslateMatrix({uvTranslation_.x, uvTranslation_.y, 0.0f});

  materialData_->uvTransform =
      Multiply(uvScaleMatrix, Multiply(uvRotateMatrix, uvTranslateMatrix));
}
...
void Sprite::Draw() {
  ...
  spriteCommon_->GetDxCommon()
      ...
      ->SetGraphicsRootConstantBufferView(
          indices.at("gMaterial"), materialResource_->GetGPUVirtualAddress());
  ...
  // SRV設定
  srvManager_->SetGraphicsRootDescriptorTable(indices.at("gTexture"),
                                              textureIndex_);

  // 描画コール
  spriteCommon_->GetDxCommon()->GetCommandList()->DrawIndexedInstanced(
      static_cast<UINT>(numVertices_), 1, 0, 0, 0);
}
...
void Sprite::VertexResource() {
  vertexResource_ = spriteCommon_->GetDxCommon()->CreateBufferResource(
      sizeof(VertexData) * static_cast<size_t>(numVertices_));

  vertexBufferView_.BufferLocation = vertexResource_->GetGPUVirtualAddress();
  vertexBufferView_.SizeInBytes =
      sizeof(VertexData) * static_cast<UINT>(numVertices_);
...
void Sprite::IndexResource() {
  indexResource_ = spriteCommon_->GetDxCommon()->CreateBufferResource(
      sizeof(uint32_t) * static_cast<size_t>(numVertices_));

  indexBufferView_.BufferLocation = indexResource_->GetGPUVirtualAddress();
  indexBufferView_.SizeInBytes =
      sizeof(uint32_t) * static_cast<UINT>(numVertices_);
