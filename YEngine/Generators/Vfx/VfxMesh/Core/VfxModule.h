...
// 追加する動きは VfxModuleType に1つ、EvaluateModules() に1 case
// 足すだけで、NoiseVolume/LightningBolt/ShockwaveRing など全 Mesh に適用できる。

// 「動きの計算」はここ（データ）＋ EvaluateModules（唯一の評価関数）に集約し、
...
namespace YoRigine {
    ...
    enum class VfxModuleType : uint8_t
    {
        Lifetime = 0,  // 爆発ワンショットの寿命を定義（age/duration で 0→1 に進行）
        Move,           // velocity 方向へ基準位置を等速移動
        Rise,           // 上方向へ上昇（velocity.y * amplitude）
        ScalePulse,          // スケールを sin で脈動（amplitude=振幅 / frequency=Hz）
    ...
    // このモジュールを適用するエレメント。All なら全効果に効く。
    // 個別に指定すると「ノイズボリュームだけ上昇 / 衝撃波リングは固定」のような作り分けができる。
    // （エレメント個別の modules では常にそのエレメントにだけ効く）
    enum class VfxModuleTarget : uint8_t
    {
        All = 0,
        NoiseVolume,
        LightningBolt,
    ...
    struct VfxModule
    {
        VfxModuleType   type      = VfxModuleType::Lifetime;
        VfxModuleTarget target    = VfxModuleTarget::All;   // 適用対象
        VfxEase         ease      = VfxEase::Linear;        // 補間カーブ

        // ---- タイミング（全タイプ共通。UE5 の Emitter ループ相当） ----
        float startTime = 0.f;   // 開始遅延(秒)。この時刻までモジュールは不発
        float window    = 0.f;   // 効果時間(秒)。0=無限（ワンショット時は寿命全体）

        // ---- タイプ別パラメータ ----
        float           duration  = 2.0f;               // Lifetime: 寿命(秒)
        Vector3         velocity  = { 0.f, 0.f, 0.f };  // Move/Rise/Accelerate: 速度 / Orbit: 回転軸
        Vector3         acceleration = { 0.f, 0.f, 0.f };// Accelerate: 加速度(重力など)
        float           amplitude = 1.0f;               // ScalePulse/Shake/BeamPulse: 振幅 / Rise: 係数 / Orbit: 半径 / Flicker: 深さ
        float           frequency = 1.0f;               // ScalePulse/Shake/Orbit/Flicker/BeamPulse: 周波数(Hz)
