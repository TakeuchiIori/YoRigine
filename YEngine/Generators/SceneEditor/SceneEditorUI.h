...
public:
  ...
  void SetColliderSelectedOnlyFlag(bool *flag) {
    showColliderSelectedOnly_ = flag;
  }
  ...
  // スタンプモード: 起動 / 状態確認 / 終了 (SceneEditor が StampMode を所有)
  void SetStartStampCallback(std::function<void()> cb) {
    startStampCallback_ = cb;
  }
  void SetStampActiveQuery(std::function<bool()> cb) { stampActiveQuery_ = cb; }
  void SetExitStampCallback(std::function<void()> cb) {
    exitStampCallback_ = cb;
  }
...
private:
  ...
  // コライダーデバッグ描画（SceneEditor のフラグ参照）
