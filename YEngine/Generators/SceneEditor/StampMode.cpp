...
void StampMode::Update(const ImVec2 &viewportPos, const ImVec2 &viewportSize) {
  ...
  // 左クリックで実体化。
  // WantCaptureMouse はチェックしない: ゲームビュー自体が ImGui
  // ウィンドウ内のため 常に true になり、全クリックが弾かれてしまう
  // (ObjectSelector も同様の理由で外している)。 ビューポート範囲チェックは
  // inView で済ませているのでそれで十分。
...
// ============================================================
// ゴースト描画 (SceneEditor::Draw から呼ばれる)
// ============================================================
void StampMode::DrawGhost() {
  if (!active_ || !ghost_ || !ghostWT_ || !camera_ || !lastHitValid_)
    return;

  // 直前のインスタンス描画 (InstancedObject3d::Flush) で別の RS/PSO
  // がバインドされている。 Object3d::Draw が参照する Root
  // パラメータ配置に戻すため、ここで明示的に再設定する。
...
// 内部: スクリーン座標 → ワールド空間レイ
// ============================================================
bool StampMode::ScreenToWorldRay(float relX, float relY, float viewW,
                                 float viewH, Vector3 &outOrigin,
                                 Vector3 &outDir) const {
  if (!camera_ || viewW <= 0.0f || viewH <= 0.0f)
    return false;
...
// ============================================================
bool StampMode::ComputePlacementPoint(const ImVec2 &viewportPos,
  ...
  Vector3 origin{}, dir{};
  if (!ScreenToWorldRay(relX, relY, viewportSize.x, viewportSize.y, origin,
                        dir)) {
    return false;
  }
  ...
  if (cm) {
    ...
    if (wasEnabled)
      srcCol->SetCollisionEnabled(false);

    const bool isHit = cm->Raycast(ray, 10000.0f, &hit, {});

    if (wasEnabled)
      srcCol->SetCollisionEnabled(true);
