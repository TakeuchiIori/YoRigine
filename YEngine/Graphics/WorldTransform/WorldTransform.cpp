...
void WorldTransform::Initialize() {
  constBuffer_ =
      Object3dCommon::GetInstance()->GetDxCommon()->CreateBufferResource(
...
void WorldTransform::UpdateMatrix() {
  Matrix4x4 rotM = useQuaternion_ ? MakeRotateMatrix(quaternion_)
  ...
  if (useAnchorPoint_) {
    ...
    matWorld_ = MakeScaleMatrix(scale_) * rotM *
                MakeTranslateMatrix(anchoredTranslation);
  } else {
    matWorld_ =
        MakeScaleMatrix(scale_) * rotM * MakeTranslateMatrix(translate_);
  }
...
/// アンカーポイント取得
/// </summary>
const Vector3 &WorldTransform::GetAnchorPoint() const { return anchorPoint_; }
...
/// </summary>
Vector3 WorldTransform::ScaleRotateToAnchor(const Vector3 &point,
                                            const Vector3 &scale,
