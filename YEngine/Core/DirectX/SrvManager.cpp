...
void SrvManager::Initialize(YoRigine::DirectXCommon *dxCommon) {
  ...
  descriptorHeap_ = dxCommon_->GetDescriptorHeap()->CreateDescriptorHeap(
      D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, kMaxSRVCount_, true);
...
void SrvManager::PreDraw() {
  ...
  dxCommon_->GetCommandList()->SetDescriptorHeaps(_countof(descriptorHeaps),
                                                  descriptorHeaps);
}
...
/// SRV を 1 個だけ確保
/// </summary>
uint32_t SrvManager::Allocate() { return Allocate(1); }
...
/// まだ SRV を確保できるか判定
/// </summary>
bool SrvManager::IsAllocation() { return (kMaxSRVCount_ > useIndex_); }
...
/// レンダーテクスチャ用 SRV の作成
/// </summary>
void SrvManager::CreateSRVforRenderTexture(uint32_t srvIndex,
                                           ID3D12Resource *pResource) {
  D3D12_SHADER_RESOURCE_VIEW_DESC srvDesc{};
...
/// 深度テクスチャ用 SRV の作成
/// </summary>
void SrvManager::CreateSRVforDepth(uint32_t srvIndex,
                                   ID3D12Resource *pResource) {
  // リソースのフォーマット取得
...
/// </summary>
void SrvManager::CreateSRVforDepthArray(uint32_t srvIndex,
                                        ID3D12Resource *pResource,
                                        UINT arraySize) {
  const auto resourceDesc = pResource->GetDesc();
  DXGI_FORMAT resourceFormat = resourceDesc.Format;
...
/// SRGBフォーマットでは UAV を作れないため、format には UNORM
/// 等の対応フォーマットを指定する。 呼び出し側は TYPELESS
/// リソースを用意し、RTV/SRV/UAV それぞれに適切な型付きビューを張る想定。
/// </summary>
void SrvManager::CreateUAVforRenderTexture(uint32_t uavIndex,
                                           ID3D12Resource *pResource,
                                           DXGI_FORMAT format) {
  D3D12_UNORDERED_ACCESS_VIEW_DESC uavDesc{};
