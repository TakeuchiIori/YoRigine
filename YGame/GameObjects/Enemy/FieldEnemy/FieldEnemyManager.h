...
struct EncountInfo {
  ...
  // 実際に接触した個体。バトル終了処理でこの個体だけを撃破／リセットする。
  // 未初期化のまま読むと不定値になるので既定値を持たせる。
  FieldEnemy *encounteredEnemy = nullptr;
...
// エンカウント発生時のコールバック
using EncounterDetailCallback =
    std::function<void(const EncountInfo &encounterInfo)>;

///************************* フィールド上の敵に関するファイルパス
///*************************///
namespace FieldEnemyPaths {
const std::string Spawn = "Resources/Json/FieldEnemies/field_spawn_data.json";
const std::string EnemyData =
    "Resources/Json/FieldEnemies/field_enemy_data.json";
} // namespace FieldEnemyPaths
...
public:
  ...
  // 通常の Line 描画パスから呼ぶ。ImGuizmo ハンドルは Editor
  // のギズモコールバック側で描画する。
  ...
  // リスポーン設定があればキューに積む（積んだら true）
  bool EnqueueRespawnIfNeeded(const std::string &enemyGroup);
...
public:
  ...
  // 敵 (グループ) 撃破時に発火するコールバック。EventTrigger / OpenGateAction
  // から購読する用途。
  using EnemyDefeatedCallback =
      std::function<void(const std::string &enemyGroup)>;
  ...
  // 指定範囲内の敵を取得
  std::vector<FieldEnemy *> GetFieldEnemiesInRange(const Vector3 &center,
                                                   float range);
  ...
  ///************************* エディター機能 *************************///
  // ShowEnemyEditor/ShowSpawnPointEditor/ShowEnemyDataEditor は
  // FieldEnemyEditorUI に分離済み (神クラス対策)。
...
private:
  ...
  // friend 経由で spawnDataMap_/enemyDataMap_ 等の private
  // メンバへアクセスする。
  ...
  // スポーンポイントIDの空きを探す (LoadEnemySpawnData
  // での重複/空ID救済にも使うため残す)
...
private:
  ...
  // Editor::AddGizmoDrawCallback で取得したコールバックID (Finalize
  // で解除する用)
  int gizmoCallbackId_ = 0;

  // エディタプレビュー用: フィールド敵の描画/コリジョン/UI
  // を全て抑制してスポーン点だけ見せる
  ...
  // ImGuizmo ハンドル描画 (Editor
  // のゲームビューウィンドウ内コンテキストから呼ばれる)
