...
void BattleSpinAttackState::Enter(BattleEnemy &enemy) {
  ...
  if (auto anim = enemy.GetAnimation()) {
    ...
    anim->StartRelativeScaleAnimation(
        {1, 1, 1}, {1.3f, 0.8f, 1.3f},
        enemy.GetEnemyData().attackParams.spin.anticipationTime);
  } else {
  ...
  targetTwistRotation_ =
      startRotation_ - enemy.GetEnemyData().attackParams.spin.twistAngle;
}
