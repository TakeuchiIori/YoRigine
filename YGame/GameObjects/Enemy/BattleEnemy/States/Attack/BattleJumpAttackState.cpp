...
void BattleJumpAttackState::Enter(BattleEnemy &enemy) {
  ...
  if (auto anim = enemy.GetAnimation()) {
    ...
    anim->StartRelativeScaleAnimation({1, 1, 1}, {1.3f, 0.6f, 1.3f}, 0.3f,
...
void BattleJumpAttackState::Update(BattleEnemy &enemy, float dt) {
  ...
  if (currentTime < anticipationEndTime) {
    ...
    // しゃがみ込む動き（イージング）
    const float crouchProgress =
        std::sin(progress * 1.5708f); // 0→1の曲線（π/2まで）

    Vector3 pos = startPos_;
    pos.y = std::max(
        0.0f, startY_ - (params.anticipationCrouchDepth * crouchProgress));
    enemy.SetTranslate(pos);

    // 色を点滅させて警告
    if (!enemy.GetAnimation()) {
      const float colorPulse =
          0.5f +
          0.5f * std::sin(currentTime * params.anticipationColorPulseSpeed);
  ...
  // === フェーズ2: チャージ（溜め） ===
    ...
    // さらに沈み込む
    Vector3 pos = startPos_;
    pos.y = std::max(0.0f, startY_ - params.anticipationCrouchDepth -
                               (params.crouchDepth * chargeProgress));
    enemy.SetTranslate(pos);
  ...
  // === フェーズ3: ジャンプ ===
    ...
    if (previousTime < chargeEndTime && currentTime >= chargeEndTime) {
      if (auto anim = enemy.GetAnimation()) {
        anim->StartRelativeScaleAnimation({1.3f, 0.6f, 1.3f},
                                          {0.8f, 1.4f, 0.8f}, 0.15f,
                                          Easing::Function::EaseOutQuad);
      }
    }
    ...
    const float heightOffset =
        params.jumpHeight * (4.0f * jumpProgress * (1.0f - jumpProgress));
    ...
    if (previousTime < jumpEndTime && currentTime >= jumpEndTime) {
      if (auto anim = enemy.GetAnimation()) {
        anim->StartRelativeScaleAnimation(
            {0.8f, 1.4f, 0.8f}, {1.2f, 0.8f, 1.2f}, 0.1f,
            Easing::Function::EaseOutQuad, [enemyPtr = &enemy]() {
              if (auto a = enemyPtr->GetAnimation()) {
                a->StartRelativeScaleAnimation({1.2f, 0.8f, 1.2f},
                                               {1.0f, 1.0f, 1.0f}, 0.2f);
              }
            });
      }
