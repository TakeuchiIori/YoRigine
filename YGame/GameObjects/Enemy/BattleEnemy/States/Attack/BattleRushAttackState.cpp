...
void BattleRushAttackState::Enter(BattleEnemy &enemy) {
  ...
  if (auto anim = enemy.GetAnimation()) {
    ...
    anim->StartRelativeScaleAnimation(
        {1, 1, 1}, {1.1f, 0.9f, 0.7f},
        enemy.GetEnemyData().attackParams.rush.anticipationTime,
...
void BattleRushAttackState::Update(BattleEnemy &enemy, float dt) {
  ...
  if (currentTime < anticipationEndTime) {
    ...
    const Vector3 backwardOffset =
        -attackDir_ * params.anticipationDistance * easeProgress;
  ...
  // === フェーズ2: チャージ ===
    ...
    if (!enemy.GetAnimation()) {
      const float chargeProgress =
          (currentTime - anticipationEndTime) / params.chargeTime;
