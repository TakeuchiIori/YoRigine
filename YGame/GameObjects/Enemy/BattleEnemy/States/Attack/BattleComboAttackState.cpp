...
void BattleComboAttackState::Enter(BattleEnemy &enemy) {
  ...
  if (auto anim = enemy.GetAnimation()) {
    ...
    anim->StartRelativeScaleAnimation(
        {1, 1, 1}, {0.8f, 1.2f, 0.8f},
        enemy.GetEnemyData().attackParams.combo.anticipationTime);
  } else {
...
void BattleComboAttackState::Update(BattleEnemy &enemy, float dt) {
  ...
  const float totalComboTime =
      anticipationEndTime + (params.phaseDuration * maxHits);
  ...
  if (currentTime < anticipationEndTime && !hasPerformedAnticipation_) {
    ...
    if (player) {
      ...
      if (Length(toPlayer) > 0.01f) {
        ...
        const Vector3 backstepOffset =
            backwardDir * params.anticipationBackstepDistance * easeProgress;
  ...
  // === フェーズ2: コンボ攻撃 ===
    ...
    const bool isSubRushing =
        (!isSubCharging &&
         timeInPhase < params.subChargeTime + params.subRushTime);
    ...
    if (isSubCharging) {
      ...
      if (previousTimeInPhase < 0.0f && timeInPhase >= 0.0f) {
        if (auto anim = enemy.GetAnimation()) {
          anim->StartRelativeScaleAnimation(
              {0.8f, 1.2f, 0.8f}, {1.0f, 0.9f, 0.9f}, params.subChargeTime);
        }
      }
      ...
      // 各ヒットの突進開始時
      if (previousTimeInPhase < params.subChargeTime &&
          timeInPhase >= params.subChargeTime) {
        if (auto anim = enemy.GetAnimation()) {
          anim->StartRelativeScaleAnimation(
              {1.0f, 0.9f, 0.9f}, {0.8f, 0.8f, 1.2f}, params.subRushTime);
        }
      }

      const float currentMultiplier =
          params.rushSpeedMultiplier + (comboCount_ * 1.0f);
...
void BattleComboAttackState::ExecuteRush(BattleEnemy &enemy,
                                         float speedMultiplier, float dt) {
  Player *player = enemy.GetPlayer();
  if (!player)
    return;
  Vector3 toPlayer = player->GetTranslate() - enemy.GetTranslate();
