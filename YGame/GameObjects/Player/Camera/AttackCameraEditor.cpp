...
void AttackCameraEditor::DrawKeyframeTimeline() {
    ...
    if (work.resetOnFinish) {
        ...
        ImGui::DragFloat("##ret", &work.returnDuration, 0.01f, 0.0f, 2.0f, "復帰%.2fs");
    ...
    // 再生カーソル
    if (isPreviewPlaying_ && component_->IsPlaying()) {
    ...
    if (work.resetOnFinish && work.returnDuration > 0.0f) {
        // タイムラインは totalDuration が 100%。returnDuration ゾーンはタイムラインの右外に帯として表示
