...
private:
	Vector3 ResolveCastOrigin(Player& player) const;
	Vector3 ResolveForward(Player& player) const;
	Vector3 ResolveTargetPoint(Player& player, const MagicTimelineEvent& event, const Vector3& origin) const;
	...
	std::string ResolveAssetName(const MagicTimelineEvent& event, const std::string& fallback) const;
};
